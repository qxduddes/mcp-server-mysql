"""Server entry point — FastMCP instance, transport dispatch, HTTP auth.

Lives inside the package so the console script (`mysql-mcp`, declared in
pyproject [project.scripts]) works when the package is installed from git or
PyPI via uvx/pipx. The thin `server.py` shim at the repo root re-exports this
module for `python server.py` (Docker) and local runs.
"""
...
def main() -> None:
    """Console-script entry point (`mysql-mcp`) and `python server.py` target.

    Logging goes to stderr — safe for stdio transport, where stdout carries
